...
Change `index.html`, commit and push. The app checks for a new copy each time it opens with a connection. Her saved data is not touched. If a change does not show up, bump `VERSION` in `sw.js` (for example to `daybook-v2`) and push again.
